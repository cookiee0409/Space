...
바다 버전은 만화 그림체(툰 셰이딩) 스타일이며, 하단의 **Pearl Rush** 미니게임이 포함되어 있습니다.

- 게임 시작을 누르면 바다 곳곳에 진주 12개와 해파리 7마리가 나타납니다.
- 90초 안에 진주를 모으면 점수를 얻고(+100점, +3초), 6초 안에 연속으로 모으면 콤보 배수가 붙습니다.
- 해파리에 닿으면 8초가 깎이고 콤보가 끊깁니다. 진주와 해파리는 소나에 금색/분홍색 점으로 표시됩니다.
- 진주를 전부 모으면 남은 시간 × 10점이 보너스로 더해지며, 최고 기록은 브라우저에 저장됩니다.
